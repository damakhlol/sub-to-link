...
async def send_photo(update: Update, photo_url: str, caption: str = ""):
    try:
        await update.message.reply_photo(photo=photo_url, caption=caption, parse_mode=ParseMode.HTML)
    except NetworkError as e:
        print(f"خطای شبکه در ارسال عکس: {e}")
        await asyncio.sleep(5)
        try:
            await update.message.reply_photo(photo=photo_url, caption=caption, parse_mode=ParseMode.HTML)
        except Exception as e:
            print(f"خطا در ارسال عکس پس از تلاش مجدد: {e}")
...
async def process_links(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    config_count = len(valid_links)
    success_message = f"تعداد {config_count} کانفیگ با موفقیت استخراج شدند✅"
    ...
    if query.data == "single":
        ...
        combined_links = "\n\n".join([f"<code>{link}</code>" for link in valid_links])
        msg = f"<b>لینک‌های اتصال:</b>\n{combined_links}"
        combined_text = "\n".join(valid_links)
        qr_url = f"https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={requests.utils.quote(combined_text)}"
        
        await query.message.reply_text(msg, parse_mode=ParseMode.HTML)
        await query.message.reply_photo(qr_url, caption="📜 QR کد تمام لینک‌ها")
        await query.message.reply_text("━━━━━━━━━━━━━━")
    
    try:
        with open(config_file, "rb") as f:
            await query.message.reply_document(document=f, filename="configs.txt", caption=success_message)
    except Exception as e:
    ...
    results = [
        InlineQueryResultArticle(
            ...
            input_message_content=InputTextMessageContent(
                "کانفیگ‌ها دریافت شد، جهت ارسال روی این دکمه کلیک کنید"
            ),
...
async def inline_process_links(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not link:
        try:
            ...
            print(f"خطا در ارسال پیام خطا به {chat_id}: {e}")
        return
    ...
    for attempt in range(2):
        try:
            ...
            print(f"خطای شبکه در ارسال فایل به {chat_id}: {e}")
            if attempt == 0:
                await asyncio.sleep(5)
                continue
            try:
                if query.message:
                    await query.message.reply_text("❌ خطا در ارسال فایل: شبکه در دسترس نیست 😔")
                else:
                    await context.bot.send_message(chat_id=chat_id, text="❌ خطا در ارسال فایل: شبکه در دسترس نیست 😔")
            except Exception as e:
                print(f"خطا در ارسال پیام خطا به {chat_id}: {e}")
        except Exception as e:
